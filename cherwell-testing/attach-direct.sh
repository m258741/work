...
env="dev"
tmpJson=/tmp/token-response.json
inputFile=$tmpJson
...
# save the json, for testing purposes
echo "$authResponse" > $tmpJson

export TOKEN=$(echo $authResponse | jq .access_token | sed 's/"//g')
set +x

# Do query
fileSize=$(expr `wc -c $inputFile | cut -f1 -d' '` - 1)
#fileSize=61
echo 'File Size(${inputFile}): $fileSize'
set -x
curl -X POST --header "Content-Type: application/octet-stream" --header "Accept: application/json" \
    --header "Authorization: Bearer ${TOKEN}" \
    -d "`cat ${inputFile}`" \
    "https://maximustest.cherwellondemand.com/CherwellAPI/api/V1/uploadbusinessobjectattachment/filename/${inputFileName}/busobid/934ec7a1701c451ce57f2c43bfbbe2e46fe4843f81/publicid/${myCR}/offset/0/totalsize/${fileSize}"
