...
#curl -s -k -X GET -H "Cmdbuild-authorization:$id" -H "Content-Type:application/json" ${sw_url} > $outfile
# Note: the 'join' strips the keynames (side effect)
#curl -s -k -X GET -H "Cmdbuild-authorization:$id" -H "Content-Type:application/json" ${sw_url} | jq --arg quote '"' '.data[] | join("~")' > $outfile
curl -s -k -X GET -H "Cmdbuild-authorization:$id" -H "Content-Type:application/json" ${sw_url} | jq '.data[0] | keys_unsorted | join(",")' | tr -d '"' #> $outfile
